...
SWEP.IronsightsAccuracyMultiplier = 0.3
...
SWEP.IronSightSpeedMultiplier = 0.3
...
local sparkle = CLIENT and CreateConVar("predator_crazy_sparks", "0", FCVAR_ARCHIVE)

-- crosshair
if CLIENT then
   local sights_opacity = CreateConVar("predator_ironsights_crosshair_opacity", "0.8", FCVAR_ARCHIVE)
   local crosshair_brightness = CreateConVar("predator_crosshair_brightness", "1.0", FCVAR_ARCHIVE)
   local crosshair_size = CreateConVar("predator_crosshair_size", "1.0", FCVAR_ARCHIVE)
   local disable_crosshair = CreateConVar("predator_disable_crosshair", "0", FCVAR_ARCHIVE)
   ...
   function SWEP:DrawHelp()
      ...
      local translate = false -- data.translatable
...
function SWEP:GetIronsightsAccuracyMultiplier()
   return self.IronsightsAccuracyMultiplier
end

function SWEP:GetPrimaryCone()
   local cone = self.Primary.Cone or 0.2
   -- 10% accuracy bonus when sighting
   return self:GetIronsights() and (cone * self:GetIronsightsAccuracyMultiplier()) or cone
end
...
function SWEP:GetIronSightSpeedMultiplier()
   return self.IronSightSpeedMultiplier
end

function SWEP:SetIronsights(b)
   if (b ~= self:GetIronsights()) then
      if b then
         self:GetOwner():SetWalkSpeed(self:GetOwner():GetWalkSpeed() * self:GetIronSightSpeedMultiplier())
      else
         self:GetOwner():SetWalkSpeed(self:GetOwner():GetWalkSpeed() / self:GetIronSightSpeedMultiplier())
      end
...
local predator_lowered = CreateConVar("predator_ironsights_lowered", "1", FCVAR_ARCHIVE)
...
function SWEP:GetViewModelPosition( pos, ang )
   ...
   local offset = self.IronSightsPos + (predator_lowered:GetBool() and LOWER_POS or vector_origin)

CreateConVar("predator_slam_max", 5, {FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE, FCVAR_REPLICATED}, "Maximum amount of SLAM's everyone can carry.")
CreateConVar("predator_slam_bought", 2, {FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE, FCVAR_REPLICATED}, "Amount of SLAM's you receive, when you buy a SLAM.")

print('yeah')

-- always derive from weapon_predatorbase
...
SWEP.Primary.ClipSize = GetConVar("predator_slam_max"):GetInt()
SWEP.Primary.DefaultClip = GetConVar("predator_slam_bought"):GetInt()
...
function SWEP:ThrowSatchel()
	...
	if (SERVER and IsValid(owner)) then
		local slam = ents.Create("predator_slam_satchel")
...
function SWEP:StickTripmine()
	...
	if (SERVER and IsValid(owner)) then
		...
		if (tr.HitWorld) then
			local slam = ents.Create("predator_slam_tripmine")
			if (IsValid(slam)) then
...
function SWEP:SecondaryAttack()
	...
	if (SERVER and self:GetActiveSatchel() > 0 and self:GetNextSecondaryFire() <= CurTime() and IsValid(owner)) then
		...
		for _, slam in pairs(ents.FindByClass("predator_slam_satchel")) do

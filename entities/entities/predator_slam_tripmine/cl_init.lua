...
local bSetting = GetConVar("predator_slam_beamsize")
	...
	if (IsValid(self)) then
		...
		hook.Add("PostDrawTranslucentRenderables", "SLAMBeam" .. index, function()
			if (IsValid(self) and self:IsActive()) then
				...
				-- render.DrawBeam(self.LaserPos, self.LaserEndPos, BeamSettings[1].size, 1, 1, BeamSettings[bSetting:GetInt()].color)
			end
		end)

...
CreateConVar("predator_fire_fallback", "0", FCVAR_ARCHIVE)
...
function ENT:Initialize()
   ...
   if CLIENT and GetConVar("predator_fire_fallback"):GetBool() then
...
function StartFires(pos, tr, num, lifetime, explode, dmgowner)
   for i=1, num do
      ...
      local flame = ents.Create("predator_flame")
      flame:SetPos(pos)
...
function ENT:Think()
   ...
   if self.dietime < CurTime() then
      if self:GetExplodeOnDeath() then
         ...
         if not success then
            ErrorNoHalt("ERROR CAUGHT: predator_flame: " .. err .. "\n")
         end
      end
   ...
   if IsValid(self.firechild) then
      if self.next_hurt < CurTime() then
         ...
         dmg:SetDamage(math.random(FIRE_DAMAGE_MIN,FIRE_DAMAGE_MAX))
